...
public:
	...
	void addTransformer(Transformer<T, N>* trans);
	void addTransformer(const Transformer<T, N>& trans);

	const Transformer<T, N>& operator[](int index)const;
	Transformer<T, N>& operator[](int index);
...
TT
void Transformations<T, N>::addTransformer(Transformer<T, N>* trans)
{
	if (getSize() >= getCapacity())
		resize(getCapacity()*2);

	this->container[this->size++] = trans;
	trans = nullptr;
}
...
TT
const Transformer<T, N>& Transformations<T, N>::operator[](int index) const
{
	return this->container[index];
}

TT
Transformer<T, N>& Transformations<T, N>::operator[](int index)
{
	return this->container[index];
}
